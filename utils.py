...
def add_asset_form():
    ...
    tipo = st.selectbox("Tipo de activo", ["Acción/ETF", "Cripto", "Bono", "Otro"])
    ...
    if not precio_mostrar:
        precio_mostrar = st.number_input("Precio", min_value=0.0, format="%.4f")
    ...
    # Comisiones
    usa_comision = st.checkbox("¿Incluye comisión?")
    comision_pct = 0.0
    if usa_comision:
